...
```{r}
# Tree in Tree is a machine learning method for constructing a decision graph.  The decision graph is built by an iterative process of the form grow-merge-grow-merge and so on. The growing phase is done by replacing internal and leaf nodes by micro decision trees, and in the merging phase that micro decision tree in merged into the full decision graph. In that way the Tree in Tree (TnT) is forming a DAG which results in a more efficient model.
# CART use binary trees (each internal node has two children and each node can have only one parent), thus In a DT, the nodes are not shared across different paths. However, this is not the case in the TnT method since TnT allows a situation where a node has multiple parents nodes, this implies that the DAG constructed by TnT can combine the nodes with similar behaviours to reduce model complexity and make it more efficient. also, by sharing nodes between different paths in the constructed decision graph, it enables a more compact model with less splits than in the DT case, as we can see in the example given in figure 2, where a DT requires 6 binary splits to classify the data and a TnT decision graph requires only 4 splits. 
```
...
#earth::earth(y ~ x1*x2, data = df, degree = 2, thresh = 0, penalty = -1)

earth_model <- earth(y ~ x1*x2, data = df, degree = 2, thresh = 0, penalty = -1)

summary(earth_model)
plotmo(earth_model)
predictions <- predict(earth_model, newdata = df)
predictions
```
